...
class RustDeskMultiWindowManager {
  ...
  openMonitorSession(
    ...
    if (unilinkMacWindow == null && windowIDs.length > 1) {
    ...
    if (unilinkMacWindow != null) {
      params['unilink_mac_window'] = unilinkMacWindow;
    }
    if (screenRect != null) {
      params['screen_rect'] = {
        'l': screenRect.left,
  ...
  Future<MultiWindowCallResult> newSession(
    ...
    if (pendingUploadPaths != null && pendingUploadPaths.isNotEmpty) {
      params['pendingUploadPaths'] = pendingUploadPaths;
    }
    ...
    final hasPendingUpload = type == WindowType.FileTransfer &&
        pendingUploadPaths != null &&
        pendingUploadPaths.isNotEmpty;

    if (!hasPendingUpload && (windows.length > 1 || !openInTabs)) {
      for (final windowId in windows) {
        if (await DesktopMultiWindow.invokeMethod(
  ...
  Future<MultiWindowCallResult> newFileTransfer(
    ...
    return await newSession(
      ...
      isSharedPassword: isSharedPassword,
      connToken: connToken,
      pendingUploadPaths: pendingUploadPaths,
    );
  }
