...
class FileManagerPage extends StatefulWidget {
  ...
  final List<String> pendingUploadPaths;
  ...
  void enqueuePendingUploadPaths(List<String> paths) {
    final state = _lastState.value;
    if (state is _FileManagerPageState) {
      state.enqueuePendingUploadPaths(paths);
    }
  }
...
class _FileManagerPageState extends State<FileManagerPage>
  ...
  late FFI _ffi;
  Timer? _pendingUploadTimer;
  bool _pendingUploadStarted = false;
  ...
  void initState() {
    ...
    _pendingUploadPaths.addAll(widget.pendingUploadPaths);
    if (_pendingUploadPaths.isNotEmpty) {
      _schedulePendingUpload();
    }
  }
  ...
  void enqueuePendingUploadPaths(List<String> paths) {
    ...
    _pendingUploadPaths.addAll(cleanPaths);
    _pendingUploadStarted = false;
    _schedulePendingUpload();
  }
  ...
  void handleDragDone(DropDoneDetails details, bool isLocal) {
    ...
    for (var file in details.files) {
      final entry = _entryFromLocalPath(file.path);
      if (entry != null) {
        items.add(entry);
      }
    }
  ...
  void _schedulePendingUpload() {
    _pendingUploadTimer?.cancel();
    _pendingUploadTimer =
        Timer.periodic(const Duration(milliseconds: 450), (_) {
      unawaited(_tryStartPendingUpload());
    });
    unawaited(_tryStartPendingUpload());
  }

  Future<void> _tryStartPendingUpload() async {
    if (_pendingUploadStarted || _pendingUploadPaths.isEmpty) return;
    final localReady = model.localController.directory.value.path.isNotEmpty;
    final remoteReady = model.remoteController.directory.value.path.isNotEmpty;
    if (!localReady || !remoteReady) return;
    _pendingUploadStarted = true;
    _pendingUploadTimer?.cancel();

    final items = SelectedItems(isLocal: true);
    for (final filePath in _pendingUploadPaths) {
      final entry = _entryFromLocalPath(filePath);
      if (entry != null) {
        items.add(entry);
      }
    }
    ...
    if (items.items.isEmpty) {
      showToast(translate('No valid files to upload'));
      return;
    }
    ...
    try {
      ...
      showToast(translate('Upload started'));
    } catch (e) {
      showToast('${translate('Upload failed')}: $e');
    }
  }

  Entry? _entryFromLocalPath(String filePath) {
    ...
    final size = isDirectory ? 0 : File(normalized).lengthSync();
    return Entry()
      ..path = normalized
