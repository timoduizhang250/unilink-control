...
class _ConnectionTabPageState extends State<ConnectionTabPage> {
  ...
  int? _display;
  final Map<String, UniLinkMacWindowInfo> _macWindowTargets = {};
  ...
  _ConnectionTabPageState(Map<String, dynamic> params) {
    ...
    final macWindowTarget = _parseMacWindowTarget(params['unilink_mac_window']);
    _isScreenRectSet = screenRect != null;
    _display = display as int?;
    tryMoveToScreenAndSetFullscreen(screenRect);
    if (peerId != null) {
      if (macWindowTarget != null) {
        _macWindowTargets[peerId!] = macWindowTarget;
        WindowController.fromWindowId(params['windowId'])
            .setTitle(_windowTitle(peerId!));
      }
      ...
      tabController.onSelected = (id) {
        ...
        WindowController.fromWindowId(params['windowId']).setTitle(
          _windowTitle(id),
        );
      ...
      tabController.add(TabInfo(
        ...
        page: RemotePage(
          ...
          macWindowTarget: macWindowTarget,
        ),
      ));
  ...
  Future<dynamic> _remoteMethodHandler(call, fromWindowId) async {
    ...
    if (call.method == kWindowEventNewRemoteDesktop) {
      ...
      final macWindowTarget = _parseMacWindowTarget(args['unilink_mac_window']);
      final preferredContentSize =
          parseParamUniLinkMacWindowContentSize(args['unilink_mac_window']);
      final prePeerCount = tabController.length;
      if (macWindowTarget != null) {
        _macWindowTargets[id] = macWindowTarget;
      }
      Future.delayed(Duration.zero, () async {
        ...
        Future.delayed(Duration(milliseconds: isWindows ? 100 : 0), () async {
          WindowController.fromWindowId(windowId()).setTitle(_windowTitle(id));
  ...
  UniLinkMacWindowInfo? _parseMacWindowTarget(dynamic value) {
    if (value is! Map) return null;
    return UniLinkMacWindowInfo.fromJson(
      value.map((key, value) => MapEntry('$key', value)),
    );
  }

  String _tabLabel(String id) => _macWindowTargets[id]?.displayTitle ?? id;

  String _windowTitle(String id) {
    final target = _macWindowTargets[id];
    if (target == null) return getWindowNameWithId(id);
    return '${translate('Mac window mode')} - ${target.displayTitle}';
  }
}
