...
openMonitorInNewTabOrWindow(
  ...
  PeerInfo pi, {
  Rect? screenRect,
  Map<String, dynamic>? unilinkMacWindow,
}) {
  ...
  if (unilinkMacWindow != null) {
    args['unilink_mac_window'] = unilinkMacWindow;
  }
  DesktopMultiWindow.invokeMethod(
...
setNewConnectWindowFrame(int windowId, String peerId, int preSessionCount,
    ...
    {Size? preferredContentSize}) async {
  if (screenRect == null) {
    ...
    if (preSessionCount == 0) {
      if (preferredContentSize == null) {
        ...
        await setNewConnectWindowPreferredContentSize(
            windowId, preferredContentSize);
      }
    }
  } else {
...
setNewConnectWindowPreferredContentSize(
    int windowId, Size preferredContentSize) async {
  if (preferredContentSize.width <= 0 ||
      preferredContentSize.height <= 0 ||
      !preferredContentSize.width.isFinite ||
  ...
  const maxContentHeight = 820.0;
  final fitScale = min(
    maxContentWidth / preferredContentSize.width,
    maxContentHeight / preferredContentSize.height,
  );
  final fillScale = max(
    minContentWidth / preferredContentSize.width,
    minContentHeight / preferredContentSize.height,
  );
  final scale = fitScale < 1.0 ? fitScale : max(1.0, fillScale);
  final contentWidth = preferredContentSize.width * scale;
  final contentHeight = preferredContentSize.height * scale;
  final frameWidth =
      (contentWidth + CanvasModel.leftToEdge + CanvasModel.rightToEdge)
          .clamp(720.0, 1500.0)
          .toDouble();
  final frameHeight =
      (contentHeight + CanvasModel.topToEdge + CanvasModel.bottomToEdge)
          .clamp(460.0, 980.0)
          .toDouble();
  final wc = WindowController.fromWindowId(windowId);
  final current = await wc.getFrame();
  await wc.setFrame(Rect.fromLTWH(
    current.center.dx - frameWidth / 2,
    current.center.dy - frameHeight / 2,
    frameWidth,
    frameHeight,
  ));
}
...
Size? parseParamUniLinkMacWindowContentSize(dynamic value) {
...
bool isChangePermanentPasswordDisabled() => false;
...
void checkUpdate() {
  if (!isWeb) {
    ...
    if (!bind.isCustomClient() || isUniLink) {
      platformFFI.registerEventHandler(
        ...
        if (evt['download_url'] is String) {
          stateGlobal.updateDownloadUrl.value = evt['download_url'];
        }
        if (evt['sha256'] is String) {
          stateGlobal.updateSha256.value = evt['sha256'];
        }
      });
