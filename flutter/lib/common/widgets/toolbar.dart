...
bool _supportsFileCopyAndPaste(FFI ffi) {
...
String _fileCopyAndPasteUnavailableTip(FFI ffi) {
  final ffiModel = ffi.ffiModel;
  final pi = ffiModel.pi;
  final perms = ffiModel.permissions;
  if (ffi.connType != ConnType.defaultConn) {
    return 'File download is only available in remote control sessions';
  }
  if (perms['file'] == false) {
    return 'Remote file permission is disabled';
  }
  if (!ffiModel.keyboard) {
    return 'Remote keyboard control is not available';
  }
  if (!bind.mainHasFileClipboard()) {
    return 'Local file clipboard is not available';
  }
  if (pi.platform == kPeerPlatformMacOS &&
      !pi.platformAdditions.containsKey(kPlatformAdditionsHasFileClipboard)) {
    return 'Remote Mac needs UniLink Control update for file download';
  }
  return 'File copy and paste is not available';
}

Future<void> _copyRemoteMacSelectionForDownload(FFI ffi) async {
  final sessionId = ffi.sessionId;
  final ffiModel = ffi.ffiModel;
  final pi = ffiModel.pi;
  if (pi.platform == kPeerPlatformMacOS) {
    showToast(translate('Downloading remote selection'));
    final result =
        await UniLinkRemoteSelectionDownloader.downloadFinderSelection(
      UniLinkEndpointTarget(
        peerId: ffi.id,
        label: pi.hostname,
    ...
    if (result.ok) {
      _showDownloadedSelectionDialog(ffi, result);
      return;
    }
  }
  if (!_supportsFileCopyAndPaste(ffi)) {
    showToast(translate(_fileCopyAndPasteUnavailableTip(ffi)));
    return;
  }
  if (ffiModel.viewOnly) {
    showToast(translate('View-only mode cannot download files'));
    return;
  }
  ...
  showToast(
      translate('Copied remote selection. Paste it in Windows to download.'));
}

void _showDownloadedSelectionDialog(
  FFI ffi,
  RemoteSelectionDownloadResult result,
) {
  final directory = result.directory;
  final paths = result.localPaths;
  if (directory == null || paths.isEmpty) return;
  ffi.dialogManager.show((setState, close, context) {
    ...
    return CustomAlertDialog(
      title: Text(translate('Remote files are ready')),
      content: Column(
        ...
        children: [
          ...
          GestureDetector(
            onPanStart: dragging ? null : (_) => unawaited(startDrag()),
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(translate(
                      dragging ? 'Dragging files out' : 'Drag files out')),
                ],
              ),
            ),
          ),
      ...
      actions: [
        dialogButton('Open folder', onPressed: () async {
...
List<TTextMenu> toolbarControls(BuildContext context, String id, FFI ffi) {
  ...
  if (isDefaultConn && isDesktop) {
    ...
    if (pi.platform == kPeerPlatformMacOS && perms['keyboard'] != false) {
      v.add(
        TTextMenu(
            child: Text(translate('Download selected files')),
            onPressed: () => _copyRemoteMacSelectionForDownload(ffi)),
      );
      v.add(
        TTextMenu(
            child: Text(translate('Extract Mac window')),
            onPressed: () => showUniLinkMacWindowMode(
                  target: UniLinkEndpointTarget(
                    peerId: ffi.id,
                    label: pi.hostname,
                    platform: pi.platform,
                    hostname: pi.hostname,
                  ...
                  onOpenWindow: (window) async {
                    final display =
                        uniLinkMacWindowDisplayProjection(window, pi.displays)
                                ?.displayIndex ??
                            0;
