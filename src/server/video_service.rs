...
fn get_capturer_monitor(
    ...
    let (mut origin, mut width, mut height) = (display.origin(), display.width(), display.height());
    #[cfg(target_os = "macos")]
    if let Some(rect) = scrap::unilink_window_capture_rect() {
        origin = (rect.x, rect.y);
        width = rect.width;
        height = rect.height;
        log::info!(
            "UniLink Mac window capture enabled: origin={:?}, width={}, height={}",
            origin,
            width,
            height,
        );
