...
const HANAKO_SECRET_ENC_VERSION: &str = "00";

pub fn main_get_local_option(key: String) -> SyncReturn<String> {
    SyncReturn(get_local_option(key))
}

pub fn main_get_encrypted_local_option(key: String) -> SyncReturn<String> {
    let stored = get_local_option(key.clone());
    let (plain, _, should_store) = decrypt_str_or_original(&stored, HANAKO_SECRET_ENC_VERSION);
    if should_store {
        let encrypted =
            encrypt_str_or_original(&plain, HANAKO_SECRET_ENC_VERSION, config::ENCRYPT_MAX_LEN);
        if encrypted.is_empty() && !plain.is_empty() {
            log::error!(
                "Encrypted local option `{}` exceeds the max encrypted length",
                key
            );
        } else {
            set_local_option(key, encrypted);
        }
    }
    SyncReturn(plain)
}
...
pub fn main_set_encrypted_local_option(key: String, value: String) {
    if value.is_empty() {
    ...
    let encrypted =
        encrypt_str_or_original(&value, HANAKO_SECRET_ENC_VERSION, config::ENCRYPT_MAX_LEN);
    ...
    if key == "is-printer-installed" {
        ...
        if key.starts_with("download-data-") {
            ...
            let direct_download_url = crate::common::SOFTWARE_UPDATE_DOWNLOAD_URL
                ...
                .unwrap()
                .clone();
            if !direct_download_url.is_empty() {
                return format!("url:{direct_download_url}");
            }
            #[cfg(target_os = "windows")]
            return match (
