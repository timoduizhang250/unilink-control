...
pub fn check_software_update() {
    if is_custom_client() && !is_unilink_control() {
        return;
    }
...
const UNILINK_UPDATE_MANIFEST_OPTION: &str = "unilink-update-manifest-url";
const UNILINK_UPDATE_MANIFEST_ENV: &str = "UNILINK_UPDATE_MANIFEST_URL";
const DEFAULT_UNILINK_UPDATE_MANIFEST_URL: &str =
    "https://github.com/timoduizhang250/unilink-control-releases/releases/latest/download/latest.json";

#[derive(Debug, Default, Deserialize)]
struct UniLinkUpdateAsset {
    #[serde(default)]
    url: String,
    #[serde(default)]
    sha256: String,
...
#[derive(Debug, Default, Deserialize)]
struct UniLinkUpdateManifest {
    #[serde(default)]
    version: String,
    #[serde(default)]
    release_url: String,
    #[serde(default)]
    windows: HashMap<String, UniLinkUpdateAsset>,
    #[serde(default)]
    macos: HashMap<String, UniLinkUpdateAsset>,
    #[serde(default)]
    android: HashMap<String, UniLinkUpdateAsset>,
}

fn is_unilink_control() -> bool {
    get_app_name().to_lowercase().contains("unilink")
}

fn unilink_update_manifest_url() -> String {
    std::env::var(UNILINK_UPDATE_MANIFEST_ENV)
        .ok()
        .filter(|x| !x.trim().is_empty())
        .or_else(|| {
            let url = LocalConfig::get_option(UNILINK_UPDATE_MANIFEST_OPTION);
            (!url.trim().is_empty()).then_some(url)
        })
        .unwrap_or_else(|| DEFAULT_UNILINK_UPDATE_MANIFEST_URL.to_owned())
}

fn current_unilink_release_arch() -> Option<&'static str> {
...
fn github_release_page_from_asset_url(asset_url: &str, version: &str) -> Option<String> {
    let needle = format!("/releases/download/{version}/");
    let pos = asset_url.find(&needle)?;
    Some(format!("{}/releases/tag/{version}", &asset_url[..pos]))
}
...
async fn do_check_unilink_software_update() -> hbb_common::ResultType<()> {
    ...
    let Some(arch) = current_unilink_release_arch() else {
        clear_software_update();
        bail!(
            "Unsupported UniLink release architecture: {}",
            std::env::consts::ARCH
        );
    };
    #[cfg(target_os = "windows")]
    let assets = &manifest.windows;
    #[cfg(target_os = "macos")]
    let assets = &manifest.macos;
    #[cfg(target_os = "android")]
    let assets = &manifest.android;
    #[cfg(not(any(target_os = "windows", target_os = "macos", target_os = "android")))]
    let assets: &HashMap<String, UniLinkUpdateAsset> = &HashMap::new();
    let Some(asset) = assets.get(arch) else {
        clear_software_update();
    ...
    if checksum.len() != 64 || !checksum.bytes().all(|byte| byte.is_ascii_hexdigit()) {
        clear_software_update();
        bail!("UniLink update asset has an invalid SHA-256 for architecture: {arch}");
    }

    let release_url = if manifest.release_url.trim().is_empty() {
        github_release_page_from_asset_url(&download_url, latest_release_version)
            .unwrap_or_else(|| download_url.clone())
    } else {
        manifest.release_url.trim().to_owned()
    };
    ...
    #[cfg(feature = "flutter")]
    {
        ...
        m.insert("url", release_url.as_str());
        m.insert("download_url", download_url.as_str());
        m.insert("sha256", checksum.as_str());
...
pub async fn do_check_software_update() -> hbb_common::ResultType<()> {
    if is_unilink_control() {
        return do_check_unilink_software_update().await;
    }
    let (request, url) =
